...
void UPauseMenu::Cancel()
{
	if (Pause)
	{
		UE_LOG(LogTemp, Warning, TEXT("Pause Timer is set"));
		this->PlayAnimation(Pause, 0.0f, 1, EUMGSequencePlayMode::Reverse, 2.0f, false);
		UGameplayStatics::SetGamePaused(GetWorld(), false);
		GetWorld()->GetTimerManager().SetTimer(AnimTimer, this, &UPauseMenu::OnClosed, 1.0f, false,
		                                       Pause->GetEndTime());
	}
}
...
void UPauseMenu::OnClosed()
{
	...
	if(auto const Player = Cast<AHandPawn>(GetOwningPlayerPawn()))
	{
		Player->UnPause();
	}
