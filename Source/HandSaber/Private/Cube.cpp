...
void ACube::BeginPlay()
{
	...
	GetWorldTimerManager().SetTimer(DestroyCountdownTimer, this, &ACube::OnDestructionTimerEnd, LifespanSecondsNonCut,
	                                false);
}
...
void ACube::CutOccured(FVector CutterPlanePoint, FVector CutterPlaneNormal, int Score)
{
	// Calculate material cut params from cutter plane
	FVector BoxCenter = GetActorLocation();
	FVector CutterOffsetVector = SaberUtils::GetOrthogonalVectorPointToPlane(
		BoxCenter, CutterPlanePoint, CutterPlaneNormal);
	...
	// Start countdown until self-destruction
	GetWorldTimerManager().SetTimer(DestroyCountdownTimer, this, &ACube::OnDestructionTimerEnd, LifespanSecondsAfterCut,
	                                false);
}
...
void ACube::StartScaleTrigger()
{
	FVector NewScale = FMath::VInterpTo(CubeMesh->GetRelativeScale3D(), FVector(0, 0, 0), GetWorld()->GetDeltaSeconds(),
	                                    ScaleChangeSpeed);
	CubeMesh->SetRelativeScale3D(NewScale);
}
